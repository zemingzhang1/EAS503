def ex1(password):
    ...
    # Good password: least 8 characters, contains at least one uppercase letter, at least one lowercase
    # letter, at least one number, and at least one of the following special characters (!, @, #, $, ^).
    if len(password) >= 8:
        caps = 0
        lows = 0
        ...
        print(caps, lows, num, s)
        if caps >= 1 and lows >= 1 and num >= 1 and s >= 1:
            return True
    return False
    pass
    # END SOLUTION
...
def ex9(filename):
    ...
    for i in splits:
        s = i.split(",")
        if len(s) > 1:
            a = sum([int(i) for i in s[1:]]) / len(s[1:])
            avg.append(a)
            dict[a] = s[:1][0]

    return dict[max(avg)], max(avg), dict[min(avg)], min(avg)
    pass
    # END SOLUTION
...
def ex14(points):
    ...
    # BEGIN SOLUTION
    n = len(points)
    xl = sum([points[i][0] for i in range(0, n)]) / n
    yl = sum([points[i][1] for i in range(0, n)]) / n

    m = sum([(points[i][0] - xl) * (points[i][1] - yl) for i in range(0, n)]) / sum(
        [(points[i][0] - xl) ** 2 for i in range(0, n)])
...
def ex15(title, header, data, filename):
    ...
    # f = open("test.txt", "w")
    #
    # colW = max([len(i) for i in header])+2
    #
    # c = len(header)
    # r = len(data)
    #
...
def ex16(lst):
    ...
    return [i for i in lst if i % 3 == 0 or i % 4 == 0]  # complete this line!
    # END SOLUTION
...
def ex17(lst):
    ...
    # BEGIN SOLUTION
    # complete the following line!
    return [i * 5 if i % 2 != 0 else i * 3 for i in lst]  # complete this line!
    # END SOLUTION
